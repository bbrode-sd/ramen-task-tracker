...
export function LoginScreen() {
  const { signInWithGoogle, loading, signInError } = useAuth();
  const { t } = useLocale();
  ...
  const handleSignIn = async () => {
    try {
      ...
      // Error is already captured in AuthContext's signInError state
      // Just prevent the unhandled promise rejection
    }
  };
      ...
      <div className="relative w-full max-w-lg mx-4 animate-fade-in">
        ...
        <div className="relative bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[24px] shadow-2xl border border-white/60 dark:border-slate-700/50 overflow-hidden">
          ...
          <div className="p-8 sm:p-10 lg:p-12">
            ...
            {signInError && (
              <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-2xl">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm text-red-700 dark:text-red-300">{signInError}</p>
                </div>
              </div>
            )}
